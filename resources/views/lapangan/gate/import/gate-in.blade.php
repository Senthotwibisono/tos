...
<script>
    ...
    async function getJobData(jobNo) {
        ...
        const url = "{{route('getData.jobImportDetil')}}";
        ...
        if (response.ok) {
            ...
            if (hasil.success) {
                ...
                $('#truck_in_date').val(hasil.item.truck_in_date).trigger('change');
    ...
    async function submitGateIn(button) {
        ...
        if (result.isConfirmed) {
            ...
            const url = '{{route('gate.import.postIn')}}';
            const response = await globalResponse(data, url);
            hideButton(button);
            if (response.ok) {
                const hasil = await response.json();
