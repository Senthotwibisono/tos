...
<div class="page-content">
          ...
          <div class="row">
            <div class="col-12">
              <div class="btn-group mb-3" role="group" aria-label="Basic example">
                <a href="/spps/delivery" type="button" class="btn btn-success">
                  Data Delivery
                </a>
                <a href="/spps/customer" type="button" class="btn btn-info">
                  Data Customer
                </a>
              </div>
            </div>
  ...
  <section class="row">
    <div class="col-12">
      <div class="card">
        <div class="card-header">
          <h4 class="card-title">Spps Data Table</h4>
          <p>Lorem Ipsum is simply dummy text of the printing and typesetting industry.</p>
        </div>
        <div class="card-body">
          <!-- <div class="row">
                  ...
                  <?php foreach ($invoices as $value) { ?>
                    <?php if ($value->orderService == "spps") { ?>
                      ...
                      <tr>
                        ...
                        <td>
                          <a type="button" href="/spps/pranota?id=<?= $value->id ?>" target="_blank" class="btn btn-sm btn-warning text-white"><i class="fa fa-file"></i></a>
                        </td>
                        <td>
                          <?php if ($value->isPiutang == 1 && $value->isPaid == 1) { ?>
                            <a type="button" href="/spps/paidinvoice?id=<?= $value->id ?>" target="_blank" class="btn btn-sm btn-primary text-white"><i class="fa fa-file"></i></a>
                        ...
                        <td>
                          <?php if ($value->isPiutang == 1 && $value->isPaid == 1) { ?>
                            <a type="button" href="/spps/job?id=<?= $value->id ?>" target="_blank" class="btn btn-sm btn-info text-white"><i class="fa fa-file"></i></a>
                          <?php } else if ($value->isPiutang == 1 && $value->isPaid == 0) { ?>
                            <a type="button" href="/spps/job?id=<?= $value->id ?>" target="_blank" class="btn btn-sm btn-info text-white"><i class="fa fa-file"></i></a>
                          <?php } else if ($value->isPiutang == 0 && $value->isPaid == 1) { ?>
                            <a type="button" href="/spps/job?id=<?= $value->id ?>" target="_blank" class="btn btn-sm btn-info text-white"><i class="fa fa-file"></i></a>
...
<?php echo $__env->make('partial.spps.main', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH D:\Fdw Files\CTOS\dev\frontend\tos-dev-local\resources\views/spps/dashboard.blade.php ENDPATH**/ ?>